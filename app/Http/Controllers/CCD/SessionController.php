...
class SessionController extends BaseCCD
{
    ...
    {
        return view('ccd.session.create', ['courseId' => $courseId]);
    }
    ...
    {
        $session = CourseSession::where('id', '=', $session_id)
            ->with('course')
            ->firstOrFail();

        return view('ccd.session.update', ['data' => $session]);
    }
    ...
    function update(
        ...
        return redirect(
            route('ccd.session.index', [$institutionId, $courseSession->id]),
        )->with('message', 'Session updated');
    }
    ...
    function delete($institutionId, $session_id)
    {
        ...
        return redirect(
            route('ccd.session.index', [$institutionId, $session_id]),
    ...
    function uploadExcelQuestionCreate(
        $institutionId,
        $courseId,
        $courseSessionId,
    ) {
        return view('ccd.session.upload-excel-questions', [
            //             'courseSession' => $this->courseSessionRepository->show($institutionId, $courseSessionId),
            'courseSession' => CourseSession::whereId($courseSessionId)
                ->whereCourse_id($courseId)
                ->with('course')
                ->firstOrFail(),
        ]);
    }

    function uploadExcelQuestionStore(
        $institutionId,
        $courseId,
        $courseSessionId,
        Request $request,
        ExcelQuestionsUploadHelper $excelQuestionsUploadHelper,
    ) {
        $courseSession = CourseSession::whereId($courseSessionId)
            ->whereCourse_id($courseId)
            ->firstOrFail();

        $ret = $excelQuestionsUploadHelper->uploadQuestions(
            $request->all(),
            $courseSession,
        );
