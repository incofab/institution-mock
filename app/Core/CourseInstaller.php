...
class CourseInstaller
{
    ...
    public $coursesFolder =
        '../public/courses/' . (self::INSTALL_FROM_HTML ? 'html/' : 'json/');
    public static $imagesBaseFolder = '../public/img/content/';

    const IS_INSTALLED_SUFFIX = '.installed.zip';

    private $extractionFolder = '../public/files/content/extracted/installed-courses/';
    ...
    {
        return file_exists(
            $this->coursesFolder . $courseId . self::IS_INSTALLED_SUFFIX,
        );
    }
    ...
    function installCourse($courseId)
    {
        ...
        if ($ret[SUCCESSFUL]) {
            ...
            rename(
                $this->coursesFolder . $courseId . '.zip',
                $this->coursesFolder . $courseId . self::IS_INSTALLED_SUFFIX,
            );
            ...
            if (is_dir($extractedFilesDir)) {
                \App\Core\Helper::deleteDir($extractedFilesDir);
            }
        } else {
            DB::rollBack();
        }
    ...
    private function extractAndFormatJSONContent($contentPath, $courseId)
    {
        ...
        if (file_exists($summaryFilename)) {
            $summaryData = json_decode(
                file_get_contents($summaryFilename),
                true,
            );
        ...
        $allSessionData = json_decode(
            file_get_contents($sessionsFilename),
            true,
        );

        $sessionVal = $this->validateSessions(
            $allSessionData,
            $contentDir,
            $courseId,
        );
        ...
        foreach ($allSessionData as $sessionData) {
            $updatedSessionData = CourseSession::whereCourse_id($courseId)
                ->whereSession($sessionData['session'])
                ->first();
            ...
            $questionsData = json_decode(
                file_get_contents($questionsFilename),
                true,
            );
    ...
    // Copy images
    private function handleImagesForJSON(
        $oldSessionsData,
        $updatedSessionsData,
        $imageLocation,
        $courseId,
    ...
    private function validateSessions($allSessionData, $contentDir, $courseId)
    {
        foreach ($allSessionData as $sessionData) {
            ...
            $questionsVal = $this->validateQuestions(
                $questionsData,
                $contentDir,
            );

            if (!$questionsVal[SUCCESSFUL]) {
                return $questionsVal;
            }
        }
    ...
    function unInstallCourse($courseId)
    {
        ...
        if (!$course) {
            return [
                SUCCESSFUL => false,
                MESSAGE => htmlentities($courseId) . ' not found',
            ];
        }
        ...
        $imagesFolder = "../public/img/content/$courseId";
    ...
    static function deleteImg(
        ...
        $file =
            self::$imagesBaseFolder . "$courseId/$course_session_id/$filename";

        if ($course_session_id) {
            $file = "$file/$course_session_id";
        }
        if ($filename) {
            $file = "$file/$filename";
        }
        ...
        if (is_dir($file)) {
            \App\Core\Helper::deleteDir($file, true);
        } else {
            unlink($file);
        }
    }
