...
class PasswordReset
{
    ...
    function resetPassword($post)
    {
        ...
        $resetData = $this->passwordResetModel
            ->where('password_reset_code', '=', $post['reset_code'])
            ->where(IS_STILL_VALID, '=', true)
            ->first();

        if (!$resetData) {
            return [SUCCESSFUL => false, MESSAGE => 'Invalid Reset code'];
        }

        if (time() > $resetData[EXPIRY_TIME]) {
            return [SUCCESSFUL => false, MESSAGE => 'Reset code expired'];
        }
        ...
        if ($newpassword !== $cpassword) {
            return [SUCCESSFUL => false, MESSAGE => 'Error: Password mismatch'];
        }
    ...
    function sendResetCode($post)
    {
        ...
        $data = $this->userModel
            ->where(PHONE_NO, '=', $post['forgot_password'])
        ...
        if (!$data) {
            return [
                SUCCESSFUL => false,
                MESSAGE => 'The supplied credentials does not exist',
            ];
        }
        ...
        if (
            ...
            return [
                SUCCESSFUL => false,
                MESSAGE => 'Wait for at least 15 minutes before requesting another one.',
            ];
        }
