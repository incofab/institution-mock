...
class ExportContent
{
  ...
  function exportCourse(Course $course)
  {
    ...
    file_put_contents(
      "$baseFolder/course.json",
      json_encode($course->toArray(), JSON_PRETTY_PRINT),
    );

    file_put_contents(
      "$baseFolder/sessions.json",
      json_encode($allSessions->toArray(), JSON_PRETTY_PRINT),
    );

    if ($summary->first()) {
      file_put_contents(
        "$baseFolder/summary.json",
        json_encode($summary->toArray(), JSON_PRETTY_PRINT),
      );
    }
    ...
    foreach ($allSessions as $session) {
      ...
      file_put_contents(
        "$baseFolder/questions_{$session['session']}_{$session['id']}.json",
        json_encode($question->toArray(), JSON_PRETTY_PRINT),
      );
    }

    // The images to copy
    $imageContentFolder = '../public/img/content/' . "$courseId";
    ...
    if (is_dir($baseFolder)) {
      \App\Core\Helper::deleteDir($baseFolder);
    }
